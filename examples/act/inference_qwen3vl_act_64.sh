#!/usr/bin/env bash
# ACT inference on Qwen3-VL-8B-Instruct, 64 frames, retain_ratio=0.25.
# Defaults reproduce the main paper configuration (coverage-aware greedy on,
# residual aggregation α=0.3, same-region pushforward).
#
# Override any TASK / model / hyper-parameter via environment variables.
set -euo pipefail
...
BASE_LOG_DIR="logs/${pretrained_model}/64/${method}"
